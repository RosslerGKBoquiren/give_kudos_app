...
class DataProcessor:
    ...
    def export_customer_data(self, output_file: str, format: str = "csv") -> bool:
        ...
        try:
            # One dict-comprehension pass: look up each customer by ID and copy
            # into builtin dicts. json.dump must not receive self.customers
            # itself — the encoder calls .keys() on that mapping and raises
            # AttributeError: 'dict' object has no attribute 'keys'.
            customers_by_id = {
                customer_id: dict(self.customers[customer_id])
            ...
            if format == "csv":
                with open(output_file, "w", newline="") as file:
                    if customers_by_id:
                        first_id = next(iter(customers_by_id))
                        fieldnames = ["customer_id"] + list(
                            customers_by_id[first_id]
                        )
                        writer = csv.DictWriter(file, fieldnames=fieldnames)
                        writer.writeheader()
                        writer.writerows(
                            {"customer_id": customer_id, **record}
                            for customer_id, record in customers_by_id.items()
                        )
            elif format == "json":
                with open(output_file, "w") as file:
                    json.dump(customers_by_id, file, indent=2)
            else:
